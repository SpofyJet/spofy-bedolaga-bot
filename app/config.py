...
class Settings(BaseSettings):
    ...
    def is_platega_recurrent_enabled(self) -> bool:
        # n22: СБП-автопродление через Platega вырезано — списания не работали,
        # а кнопка обещала цену без скидки (подписка Platega всегда списывала
        # полную фиксированную сумму). Ответ False скрывает кнопки в боте и
        # кабинете, блокирует создание новых привязок и pre-bill уведомления.
        # Отмена существующих привязок и обработка их коллбеков НЕ гейтятся —
        # это операции безопасности. Баланс-автопродление и Lava не затронуты.
        return False
