...
def get_main_menu_keyboard(
    ...
    *,
    ...
    if subscription_buttons:
        # Каждая кнопка подписки — в отдельной строке
    ...
    # Пополнение игр (WATA Digital Goods)
    if getattr(settings, 'WATA_DG_TOKEN', None):
        keyboard.append([
            InlineKeyboardButton(
                text=texts.t('MENU_GAME_TOPUP', '🎮 Пополнение игр'),
                callback_data='dg:main',
            )
        ])
...
def get_info_menu_keyboard(
    ...
    # Проверяем, включена ли поддержка
    ...
    try:
        ...
        pass  # если модуль не найден или ошибка — используем значение из настроек
    ...
    # Добавляем кнопку выбора языка (если включена в настройках)
...
def get_subscription_keyboard(
    ...
    if has_subscription:
        ...
        if is_trial:
            keyboard.append(
                [InlineKeyboardButton(text=texts.t('SUBSCRIPTION_UPGRADE', '🌟 Перейти на платный тариф'), callback_data='subscription_upgrade')]
            )

        # ✅ ИЗМЕНЕНО: Убрали блок if is_trial - все кнопки доступны всем!
        ...
        if is_daily_tariff:
            ...
            # НЕ показываем для пробных (trial) подписок
            keyboard.append(
                [
        ...
        if settings.is_tariffs_mode() and subscription:
            # Для пробной подписки — открываем меню выбора тарифов (как "Перейти на платный тариф")
            # Для суточных тарифов — список тарифов, для обычных — мгновенное переключение
            if is_trial:
                tariff_callback = 'subscription_upgrade'
            elif is_daily_tariff:
                tariff_callback = 'tariff_switch'
            else:
                tariff_callback = 'instant_switch'
            settings_row.append(
                InlineKeyboardButton(
