...
class PricingEngine:
    ...
    def calculate_tariff_switch_cost(
        ...
        *,
        ...
        # Тариф с высоким уровнем (>= 8): всегда полная стоимость тарифа,
        # независимо от дневной ставки (напр. тариф "навсегда" за 4000₽/900 дней).
        new_tier = getattr(new_tariff, 'tier_level', 1) or 1
        if new_tier >= 8:
            periods = new_tariff.get_available_periods()
            if periods:
                first_period = periods[0]
                full_price = new_tariff.get_price_for_period(first_period) or 0
                if full_price > 0:
                    raw_cost = full_price
                    group_pct = 0
                    offer_pct = 0
                    if user:
                        ...
                        if promo_group is not None:
                            group_pct = promo_group.get_discount_percent('period', first_period)
